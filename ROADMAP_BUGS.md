...
- **Severity:** HIGH
- **Description:** When thinking is enabled but the model never emits `</think>`, the thinking phase is only limited by `thinking_max_tokens` (64). If the total `max_len` is 128 and thinking uses 64+ tokens, the response phase gets the remaining steps. However, if the model generates `<think>` early (step 2) and thinking runs for 64 tokens (ending at step 66), the response has 62 steps — fine. But if the model is confused and `<think>` appears late (step 100), thinking runs until step 128 and response gets 0 steps, returning thinking-only output with empty response.
