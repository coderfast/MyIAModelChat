...
class SentencePieceTokenizerWrapper:
    ...
    def _initialize_special_token_ids(self):
        ...
        # Legacy (map to GPT-2 standard for backward compat)
        self._context_id = context_id if context_id >= 0 else problem_id
        self._answer_id = answer_id if answer_id >= 0 else final_id
        self._observation_id = observation_id if observation_id >= 0 else tool_result_id
        self._observation_end_id = observation_end_id if observation_end_id >= 0 else tool_result_id

        # Log real token IDs for debugging
        for name, tid in [
            ('pad', pad_id), ('unk', unk_id), ('bos', bos_id), ('eos', eos_id),
            ('<|problem|>', problem_id), ('<|thinking|>', thinking_mode_id),
            ('<thinking>', thinking_id), ('</thinking>', thinking_end_id),
            ('<|final|>', final_id),
    ...
    def decode(self, indices, skip_special_tokens=True):
        ...
        if skip_special_tokens:
            special_ids = {id_ for id_ in (
                ...
                self._user_id, self._assistant_id,
